...
class BargainMaker
  ...
  def make_offer(config:)
    ...
    answer = get_and_validate_input(prompt: "your choice, between 1 to #{last_choice_index} >>>",
                                    possible_answers: (1..last_choice_index).to_a)
  ...
  def collect_price(price)
    ...
    when :add_extra_category
      inactive_categories = config.categories.select { |_k, v| !v[0] }
      inactive_categories[inactive_categories.keys.sample][0] = true
      @prices.delete(:add_extra_category) if config.categories.select { |_k, v| !v[0] }.count == 0
      puts 'added another category of puzzles'.red
    end
  end
end
